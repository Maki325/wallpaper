...
namespace Wallpaper {
  ...
  Application::Application()
    : m_desktopHWnd(Utils::GetWallpaperWindow()),
        m_desktopDC(GetDC(m_desktopHWnd)), m_openGL(OpenGL(m_desktopHWnd)) {
    srand(time(NULL));
  }
  ...
  void Application::Run() {
    ...
    m_openGL.SetViewport(s_monitors.at(1).area);
    Utils::PrintRect(s_monitors.at(1).area);

    Wallpaper::VideoTask videoTask("F:\\Wallpaper\\Wallpaper\\resources\\videos\\Test.mp4", 1);
    std::thread videoThread(&Wallpaper::VideoTask::Run, &videoTask);
  ...
  void Application::GetMonitors() {
    RECT windowSize;
    GetWindowRect(m_desktopHWnd, &windowSize);
    std::cout << "Window size before: ";
    Utils::PrintRect(windowSize);
    Utils::NormalizeRect(windowSize);
    std::cout << "Window size after: ";
    Utils::PrintRect(windowSize);
  ...
  BOOL Application::EnumerateMonitors(
    HMONITOR hMonitor,
    HDC hDc,
    LPRECT pRect,
    LPARAM data
  ...
  std::vector<Monitor> Application::s_monitors;
}
